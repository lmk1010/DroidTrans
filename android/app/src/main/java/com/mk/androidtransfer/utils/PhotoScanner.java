...
public class PhotoScanner {
    ...
    public static ScanResult scan(Context context) {
        ...
        for (PhotoInfo photo : photos) {
            ...
            if (album == null) {
                String displayName = AlbumInfo.getAlbumDisplayName(context, hintPath);
                if (displayName.equals(context.getString(R.string.album_other))
                        && !TextUtils.isEmpty(photo.getBucketName())) {
    ...
    private static int albumPriority(AlbumInfo album) {
        int icon = album.getIconResId();
        if (icon == R.drawable.ic_album_camera) return 0;
        if (icon == R.drawable.ic_album_screenshot) return 1;
        if (icon == R.drawable.ic_album_wechat) return 2;
        if (icon == R.drawable.ic_album_qq) return 3;
        if (icon == R.drawable.ic_album_download) return 5;
        if (icon == R.drawable.ic_album_image) return 6;
        return 10;
    }
}
