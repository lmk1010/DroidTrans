...
public class PhotoSelectionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (serverUrl == null || serverUrl.isEmpty()) {
            Toast.makeText(this, R.string.invalid_server, Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
    ...
    private void setupListeners() {
        ...
        btnDeselectAll.setOnClickListener(v -> {
            if (isAlbumView && !isAlbumSelectionMode) {
                ...
                Toast.makeText(this, R.string.select_album_hint, Toast.LENGTH_SHORT).show();
            } else if (isAlbumSelectionMode) {
                // 相册选择模式：取消全选相册
    ...
    private void setupRecyclerView() {
        ...
        recyclerViewPhotos.setItemAnimator(null);
        recyclerViewPhotos.setItemViewCacheSize(24);
        ...
        recyclerViewAlbums.setItemAnimator(null);
        recyclerViewAlbums.setItemViewCacheSize(12);
    ...
    private void loadPhotos() {
        ...
        new Thread(() -> {
            try {
                ...
                mainHandler.post(() -> {
                    showLoading(false);
                    Toast.makeText(this, getString(R.string.load_photos_failed, e.getMessage()), Toast.LENGTH_LONG).show();
                    Log.e(TAG, "加载照片失败", e);
                });
            }
        }).start();
    ...
    private void switchToPhotoView() {
        ...
        if (currentAlbum != null) {
            ...
            tvPhotoTitle.setText(R.string.all_photos);
        }
    ...
    private void enterAlbumSelectionMode() {
        ...
        // 更新UI
        tvPhotoTitle.setText(R.string.select_albums);
    ...
    private void updateAlbumSelectionCount() {
        ...
        if (count == 0) {
            chipSelectionCount.setText(R.string.selected_albums_zero);
            // 在相册选择模式下，即使为0也要显示
            chipSelectionCount.setVisibility(View.VISIBLE);
        } else {
            chipSelectionCount.setVisibility(View.VISIBLE);
            chipSelectionCount.setText(getString(R.string.selected_albums_count, count));
        }
        
        // 更新FAB文本
        if (count > 0) {
            fabUpload.setText(R.string.upload_albums);
            fabUpload.setEnabled(true);
        } else {
            fabUpload.setText(R.string.upload_photos);
    ...
    private void uploadSelectedAlbums() {
        ...
        if (selectedAlbums.isEmpty()) {
            Toast.makeText(this, R.string.select_at_least_one_album, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    private void updateSelectionCount(int count) {
        if (count == 0) {
            chipSelectionCount.setText(R.string.selected_count_zero);
            chipSelectionCount.setVisibility(isAlbumView ? View.GONE : View.VISIBLE);
        } else {
            chipSelectionCount.setVisibility(View.VISIBLE);
            chipSelectionCount.setText(getString(R.string.selected_count, count));
        }
    }
    ...
    private void showSortDialog() {
        String[] sortOptions = {getString(R.string.sort_by_time), getString(R.string.sort_by_size), getString(R.string.sort_by_name)};
        new AlertDialog.Builder(this)
    ...
    private void startUpload(List<PhotoInfo> selectedPhotos) {
        ...
        intent.putExtra("server_url", serverUrl);
        intent.putExtra("server_name", serverName != null ? serverName : getString(R.string.unknown_server));
        intent.putParcelableArrayListExtra("selected_photos", new ArrayList<>(selectedPhotos));
        intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
        startActivity(intent);
        overridePendingTransition(0, 0);
    ...
    private void toggleFilterUploaded() {
        ...
        new Thread(() -> {
            ...
            mainHandler.post(() -> {
                showLoading(false);
                applyPhotoList(currentSourcePhotos());
                btnFilterUploaded.setText(isFilteringUploaded ? R.string.show_all : R.string.exclude_uploaded);
                if (isFilteringUploaded) {
                    if (finalExcluded > 0) {
                        Toast.makeText(this, getString(R.string.excluded_uploaded, finalExcluded), Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(this, R.string.no_uploaded_files, Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(this, R.string.showing_all_files, Toast.LENGTH_SHORT).show();
                    updateFilterButtonText();
                }
            ...
            try {
                ...
                mainHandler.post(() -> {
                    if (finalUploadedCount > 0) {
                        btnFilterUploaded.setText(getString(R.string.exclude_uploaded_count, finalUploadedCount));
                    } else {
                        btnFilterUploaded.setText(R.string.exclude_uploaded);
                    }
                });
