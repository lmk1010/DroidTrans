...
差异判断用「文件名 + 大小 + 修改时间」。只看名字和大小不够：一个文本文件
改掉几个字、一张图重新导出一次，大小常常分毫不差，那样改动就再也备不进来。
算内容哈希更准，但要把每张照片整个读一遍，一次几千张会慢到没法用。
手机源问不起修改时间（要一个个 `adb stat`），那一路退回只看名字和大小——
照片一旦拍下来就不会再被就地改写，这个退化在那儿是安全的。

源有两种：USB 连着的安卓手机（扫全部相册），或本机的一个文件夹。

**备份盘必须支持硬链接。** exFAT 和 FAT32 不支持，而移动硬盘出厂基本都是
exFAT（为了 Windows/Mac 通用）。那种盘上每个快照都会退化成完整一份，
实测 20 MB 的源备三次占到 94 MB。界面会在计划卡片上直说，统计也如实按
「新增」算——报「复用」等于告诉用户没占地方，而他的硬盘正在成倍地满。
macOS 上格式化成 APFS 即可。

**第一份必须是真拷贝，不能硬链接到源文件。** 硬链接会和源共用同一个 inode，
源那边被就地改写（追加、`dd`、sqlite、有些图片编辑器）时，快照里的内容跟着
一起变——一份会随原件变化的东西不是备份。只有快照之间才可以硬链接，
那些是我们自己写出来的、不会再被谁动的文件。
