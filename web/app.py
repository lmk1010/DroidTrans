...
def _parse_adb_version(text: str) -> tuple[int, int, int]:
    m = re.search(r'Version\s+(\d+)\.(\d+)\.(\d+)', text or '')
    if m:
...
def _resolve_adb_bin() -> str:
    """选最新的 adb。Homebrew 的 35.x 会挡住 SDK 的 36.x，无线配对端口会失败。"""
    home = os.path.expanduser('~')
    candidates = [
    ...
    for p in candidates:
        ...
        try:
            out = subprocess.check_output(
                [p, 'version'], text=True, stderr=subprocess.STDOUT, timeout=3
            )
            ver = _parse_adb_version(out)
            if ver >= best_ver:
                best, best_ver = p, ver
        except Exception:
            if not best:
...
try:
    _adb_ver_lines = subprocess.check_output(
        [ADB_BIN, 'version'], text=True, stderr=subprocess.STDOUT, timeout=3
    ).strip().splitlines()
    ADB_VERSION = ' / '.join(_adb_ver_lines[:2])
except Exception:
    ADB_VERSION = 'unknown'


def _maybe_inject_serial(command: str) -> str:
    """把命令里的 adb 换成最新二进制，并按需注入 -s <serial>。"""
    try:
        cmd = command.strip()
        if cmd == 'adb' or cmd.startswith('adb '):
            cmd = ADB_BIN + cmd[3:]
        if not cmd.startswith(ADB_BIN):
            return command
        rest = cmd[len(ADB_BIN):].lstrip()
        head = rest.split(' ', 1)[0] if rest else ''
        skip = {
            'devices', 'start-server', 'kill-server', 'version',
            'pair', 'connect', 'disconnect', 'mdns',
        }
        if head in skip or not selected_device:
            return f'{ADB_BIN} {rest}'.strip()
        return f'{ADB_BIN} -s {selected_device} {rest}'
    except Exception:
        return command
...
def _adb_prefix() -> str:
    try:
        if selected_device:
            return f"{ADB_BIN} -s {selected_device}"
    except Exception:
        pass
    return ADB_BIN
